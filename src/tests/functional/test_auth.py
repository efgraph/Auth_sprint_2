...
def test_history(test_client, tokens):
    response = test_client.get('/auth/login-history', query_string={'jwt': tokens[0]})
    assert response.status_code == HTTPStatus.OK


def test_edit(test_client, tokens):
    response = test_client.post('/auth/edit-user', query_string={'jwt': tokens[0]},
                                data=dict(email='superuser@admin.com'))
    assert response.status_code == HTTPStatus.OK


def test_refresh_token(test_client, tokens):
    response = test_client.post('/auth/refresh-token', query_string={'jwt': tokens[1]})
    assert response.status_code == HTTPStatus.OK
...
def test_logout(test_client, tokens):
    ...
    response = test_client.get('/auth/logout', headers={'Access-Token': access_token, 'Refresh-Token': refresh_token})
    assert response.status_code == HTTPStatus.OK
